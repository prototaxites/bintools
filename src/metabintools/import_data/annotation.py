...
from metabintools.dataclasses.annotation import Annotation, Frame, Strand
from metabintools.dataclasses.contig import Contig


class ContigAnnotator:
    @staticmethod
    ...
    def _merge_annotations(contig: Contig, new_annotations, overwrite: bool):
        ...
        return (
            contig.annotations + new_annotations
            if contig.annotations
            else new_annotations
        )

    def annotate_contigs(
        self, contigs: dict[str, Contig], gff: Path, overwrite: bool = False
    ) -> dict[str, Contig]:
        """
